...
import yakt.StringishMap;
import yakt.StringAdapter;
import utest.Assert;

class StringishMapTest extends utest.Test {
	private var imap:IMap<Int, Int>;

	public function setup() {
		var intStringAdapter:StringAdapter<Int> = {
			toString: Std.string,
			fromString: function(val:String):Int {
				return Std.parseInt(val);
			}
		};
		var map = new StringishMap<Int, Int>(intStringAdapter);
		imap = map;
		imap.set(1, 10);
		imap.set(2, 20);
		imap.set(3, 30);
